...
> TODO : 为编写蓝图脚本的subskill提供可查询的向量数据库；将AnalysisAgent分析出的工程结构嵌入向量数据库供其他模块查询工程结构；为codeAgent接入reviewAgent代码审查功能；需要在调用codeAgent前调查整体项目结构和需要改动的代码文件依赖关系，预期单独划分一个模块，负责定期整理依赖关系；

> FUTURE : 实现在单个任务中的逐步骤记录，使思考链和调用链可追踪，对于大规模/长流程任务，实现原子级别的中断可恢复